...
public class DictionaryManagement {
  ...
  public static int dictionarySearchers(String s) throws SQLException {
    ...
    if (l + 1 > r) {
      if (Dictionary.WordList.get(r).getWord_target().equals(s)) {
        return r;
      } else {
        return -1;
      }
    } else {
      if (Dictionary.WordList.get(l + 1).getWord_target().equals(s)) {
        return l + 1;
      } else {
        return -1;
      }
  ...
  public static Word dictionarySearcherWord(String s) throws SQLException {
    if (dictionarySearchers(s) == -1) {
      return null;
    } else {
      return Dictionary.WordList.get(dictionarySearchers(s));
    }
  }
